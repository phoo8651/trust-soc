...
from metrics import INGEST_LATENCY, INGEST_REJECTIONS, INGEST_REQUESTS, record_e2e_latency
import model
import schemas
...
async def ingest_logs(
    ...
    if INGEST_REQUESTS:
        INGEST_REQUESTS.labels(client_id=client_id).inc()
    if not authorization.startswith("Bearer "):
        _record_rejection(client_id, "invalid_auth")
    ...
    if not agent_row or not validate_access(access_token, agent_row):
        _record_rejection(client_id, "unauthorized")
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="unauthorized")

    try:
        verify_timestamp(req_ts)
    except ValueError as exc:
        _record_rejection(client_id, "timestamp")
    ...
    try:
        ...
        _record_rejection(client_id, "payload_hash")
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail=str(exc)) from exc
    ...
    if replay:
        _record_rejection(client_id, "replay")
    ...
    if recent_count >= INGEST_RATE_LIMIT_PER_WINDOW:
        ...
        db.rollback()
        _record_rejection(client_id, "rate_limit")
        raise HTTPException(
    ...
    if queue_is_overloaded():
        ...
        db.rollback()
        _record_rejection(client_id, "backpressure")
        raise HTTPException(
            status_code=status.HTTP_429_TOO_MANY_REQUESTS,
            detail="backpressure",
    ...
    for raw_row, (rec, normalized, outcome) in zip(raw_rows, prepared):
        ...
        validated_refs = [ref.dict() for ref in schemas.validate_evidence_refs(event_payload["evidence_refs"])]
        event_payload["evidence_refs"] = validated_refs
        events.append(
            model.Event(
                ts=rec.ts,
                client_id=payload.meta.client_id,
...
def _record_rejection(client_id: str, reason: str) -> None:
    if INGEST_REJECTIONS:
        INGEST_REJECTIONS.labels(client_id=client_id, status=reason).inc()
