...
from metrics import JOBS_DELIVERED
import model
import schemas
...
def pull_jobs(
    ...
    for job in jobs:
        ...
        if granted < required:
            job.status = "pending"
            continue
        if job.rate_limit_per_min:
            min_interval = 60 / max(1, job.rate_limit_per_min)
            if job.last_delivered_at:
                delta = (now - job.last_delivered_at).total_seconds()
                if delta < min_interval:
                    logger.debug(
                        "job %s held due to rate limit (remaining %.2fs)",
                        job.job_id,
                        min_interval - delta,
                    )
                    continue
        ...
        if JOBS_DELIVERED:
            JOBS_DELIVERED.labels(agent_id=agent_id, job_type=job.job_type).inc()
        db.add(
            model.AuditLog(
                actor="system",
                subject=job.job_id,
